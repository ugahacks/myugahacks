...
def organizer_tabs(user):
    t = [('Applications', reverse('app_list'), False),
         ('Review', reverse('review'),
          'new' if models.Application.objects.exclude(vote__user_id=user.id).filter(status=Application.PENDING) else ''),
...
class RankingView(TabsViewMixin, IsOrganizerMixin, SingleTableMixin, TemplateView):
    ...
    def get_queryset(self):
        return (User.objects.annotate(vote_count=Count('vote__calculated_vote'))
                .annotate(skip_count=Count('vote') - Count('vote__calculated_vote'))
                .annotate(total_count=Count('vote')).exclude(total_count=0))
...
class InviteListView(TabsViewMixin, IsDirectorMixin, SingleTableMixin, FilterView):
    ...
    def get_queryset(self):
        return models.Application.annotate_vote(models.Application.objects.filter(status=Application.PENDING))
...
class ApplicationDetailView(TabsViewMixin, IsOrganizerMixin, TemplateView):
    ...
    def waitlist_application(self, application):
        try:
            ...
            m = emails.create_waitlist_email(application, self.request)
            m.send()
...
class InviteTeamListView(TabsViewMixin, IsDirectorMixin, SingleTableMixin, TemplateView):
    ...
    def get_queryset(self):
        return models.Application.objects.filter(status=Application.PENDING).exclude(user__team__team_code__isnull=True) \
...
class DubiousApplicationsListView(TabsViewMixin, IsOrganizerMixin, ExportMixin, SingleTableMixin, FilterView):
    ...
    def get_queryset(self):
        return models.Application.objects.filter(status=Application.DUBIOUS)

    def post(self, request, *args, **kwargs):
        application = models.Application.objects.get(uuid=request.POST.get('id'))
        if request.POST.get('set_contacted') and request.user.is_director:
            application.set_contacted(self.request.user)
        elif request.POST.get('unset_dubious') and request.user.is_director:
            application.unset_dubious()
        elif request.POST.get('reject') and request.user.is_director:
